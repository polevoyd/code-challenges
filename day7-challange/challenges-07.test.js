...
The returned array should containing the result of raising 2 to the power of the original input element.
...
Write a function named charCode that, given an array of letters as an input, uses map to return a new array where each element
...
const charCode = (input) => {
  ...
  {
    array[index] = element.charCodeAt(0);
  });
  return input;
};
...
const evenOdd = (input) => {
  ...
  input.forEach(function(element, index, array)
  {
    array[index] = (Number.isInteger(element)) ? (element % 2 === 0) ? 'even' : 'odd' : 'N/A';
  });
  return input;
};
...
const extractStats = (stats) => {
  ...
  return stats.map((e) => { return {name: e.stat.name, total : e.effort + e.baseStat}; });
};
